...
static ssize_t set_gpu_custom_max_clock(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((gpu_max_clock < 160) || (gpu_max_clock > 730)) {
		GPU_LOG(DVFS_WARNING, DUMMY, 0u, 0u, "%s: out of range [160~730] (%d)\n", __func__, gpu_max_clock);
		return -ENOENT;
	}

	if ((gpu_max_clock < platform->gpu_max_clock_limit) || (gpu_max_clock < platform->gpu_min_clock)) {
		GPU_LOG(DVFS_WARNING, DUMMY, 0u, 0u, "%s: Cannot be less than max clock limit or min clock\n", __func__);
		return -ENOENT;
	}
...
static ssize_t show_gpu_custom_max_clock_limit(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	...
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "%d", platform->gpu_max_clock_limit);
...
static ssize_t set_gpu_custom_max_clock_limit(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret, gpu_max_clock_limit;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;

	if (!platform)
		return -ENODEV;

	ret = kstrtoint(buf, 0, &gpu_max_clock_limit);
	...
	if ((gpu_max_clock_limit < 160) || (gpu_max_clock_limit > 730)) {
		GPU_LOG(DVFS_WARNING, DUMMY, 0u, 0u, "%s: out of range [160~730] (%d)\n", __func__, gpu_max_clock_limit);
		return -ENOENT;
	}

	if ((gpu_max_clock_limit > platform->gpu_max_clock) || (gpu_max_clock_limit < platform->gpu_min_clock)) {
		GPU_LOG(DVFS_WARNING, DUMMY, 0u, 0u, "%s: Cannot be greater than max clock or less than min clock\n", __func__);
		return -ENOENT;
	}

	platform->gpu_max_clock_limit = gpu_max_clock_limit;
...
static ssize_t set_gpu_custom_min_clock(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((gpu_min_clock < 160) || (gpu_min_clock > 730)) {
		GPU_LOG(DVFS_WARNING, DUMMY, 0u, 0u, "%s: out of range [160~730] (%d)\n", __func__, gpu_min_clock);
		return -ENOENT;
	}

	if ((gpu_min_clock > platform->gpu_max_clock) || (gpu_min_clock > platform->gpu_max_clock_limit)) {
		GPU_LOG(DVFS_WARNING, DUMMY, 0u, 0u, "%s: Cannot be greater than max clock or max clock limit\n", __func__);
		return -ENOENT;
	}
...
DEVICE_ATTR(max_clock_limit, S_IRUGO|S_IWUSR, show_gpu_custom_max_clock_limit, set_gpu_custom_max_clock_limit);
...
int gpu_create_sysfs_file(struct device *dev)
{
	...
	if (device_create_file(dev, &dev_attr_max_clock_limit)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "couldn't create sysfs file [max_clock_limit]\n");
		goto out;
	}
...
void gpu_remove_sysfs_file(struct device *dev)
{
	...
	device_remove_file(dev, &dev_attr_max_clock_limit);
